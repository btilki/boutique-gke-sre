...
| ------------------------------------------------ | --------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `github-ci@boutique-gke.iam.gserviceaccount.com` | CI build, push, sign        | Repo-scoped `roles/artifactregistry.writer` on `boutique` (via Terraform module); WIF impersonation only — no JSON keys |
